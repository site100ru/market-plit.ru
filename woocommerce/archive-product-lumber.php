				...
				<div class="row">
					<div class="col-md-3">
						<?php
							...
							// Получаем ID текущей категории
							$obj = get_queried_object();
							$current_cat_id = $obj->term_id;
							//echo $current_cat_id;
							
							// Выводим подкатегории категории "Изделия из фанеры"
							echo '<span class="widget-title">Категории древесных плит</span>';
							echo '<ul class="product-categories">'; ?>
							<li class="cat-item<?php if ( $current_cat_id == 338 ) { echo ' current-cat'; } ?>">
								<a href="https://stock-line.ru/product-category/древесные-плиты/">Все древесные плиты</a>
								<?php
									// Считаем количество товаров в категории
									$term = get_term( 338, 'product_cat' );
							...
							foreach ($product_cat as $parent_product_cat) {
								// Считаем количество товаров в категории
								$term = get_term( $parent_product_cat->term_id, 'product_cat' ); ?>
								<li class="cat-item<?php if ( $current_cat_id == $parent_product_cat->term_id ) { echo ' current-cat'; } ?>">	
								...
								<?php echo '<a href="'.get_term_link($parent_product_cat->term_id).'">'.$parent_product_cat->name.'</a>';
									...
									foreach ($child_product_cats as $child_product_cat) { ?>
										...
										<!--echo '<li class="cat-item"><a href="'.get_term_link($child_product_cat->term_id).'">'.$child_product_cat->name.'</a>';-->
										<li class="cat-item<?php if ( $current_cat_id == $child_product_cat->term_id ) { echo ' current-cat'; } ?>">	
										<?php echo '<a href="'.get_term_link($child_product_cat->term_id).'">'.$child_product_cat->name.'</a>';
										
										
										// Считаем количество товаров в категории
										$term2 = get_term( $child_product_cat->term_id, 'product_cat' );
										echo ' <span class="count">('.$term2->count.')</span>';
										echo '</li>';
									}
								echo '</li>';
							}
